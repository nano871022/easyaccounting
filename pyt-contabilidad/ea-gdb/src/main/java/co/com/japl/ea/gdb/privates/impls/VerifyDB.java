...
public class VerifyDB implements IVerifyStructuredDB{
	...
	/**
	 * Se encarga de realizar la busqueda de la clase dentro de todos los paquetes que contienen dtos
	 * @param nameDto {@link String}
	 * @return {@link Class}
	 * @throws {@link ClassNotFoundException}
	 */
	@SuppressWarnings("unchecked")
	private final synchronized <T extends ADto> Class<T> getClassFromNameFileSql(String nameDto)throws ClassNotFoundException{
		Class<T> dtoClass = null;
		try {
			var pathAllDTO = CONST_PACKAGE+ CONST_DOT+CONST_SUB_PACKAGES[posPackage] + CONST_DOT + nameDto.substring(0, 1).toUpperCase()+nameDto.substring(1)+CONST_DTO;
	...
	private final void verifyPersistenceException(Exception e, Map<String, File> map,
			String dtoName) {
