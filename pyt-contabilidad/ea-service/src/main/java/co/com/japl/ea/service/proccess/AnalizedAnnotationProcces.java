...
public class AnalizedAnnotationProcces implements Reflection {
	...
	/**
	 * Se encarga de validar si en el campo dentro del dto suministrrado es de tipo
	 * {@link ADto} debe verificar si contiene la anotacion {@link ValueInObject} si
	 * es de esta manera se debe crear una instancia de ese campo y buscar en la
	 * instancia el campo que se encuentra en la anotacion, y ingresar ddentro de
	 * esta instancia el valor en el campo anotado yy despues esa instancia ponerla
	 * en el dto suministrado y retornar en dto. En el caso que no contenga la
			...
			if (field.getType() == String.class) {
				...
				if (StringUtils.isNotBlank(value)) {
					if (value.length() >= size.max()) {
						throw new Exception(
								"El campo tiene mas caracteres de lo permitido " + value.length() + ":" + size.max());
					}
					if (value.length() <= size.min()) {
						throw new Exception(
								"El campo tiene menos caracteres de lo permitido " + value.length() + ":" + size.min());
					}
				}
			}
		}
