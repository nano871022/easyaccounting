...
public class UpdClassProccessor extends AbstractProccessorGeneric<UpdClassVerified, UpdClassGrouped, UpdClass> {
	...
	protected void proccess(RoundEnvironment roundEnv) throws Exception {
		try {
			...
			if (groupClass != null && groupClass.values() != null) {
				for (UpdClassGrouped services : groupClass.values()) {
	...
	@Override
	protected boolean isValidClass(UpdClassVerified annotationClass) {
