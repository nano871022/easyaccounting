...
public final class ValidFields {
	...
	public static final Boolean valid(TextField field, Boolean notEmpty, Integer min, Integer max,String msnError) {
		...
		if(!valid)error(field,msnError);
		if(valid)success(field);
		return valid;
	}
	...
	public static final <T extends Object> Boolean valid(ChoiceBox<T> selects, Boolean notEmpty,String msnError) {
		...
		if (notEmpty && select == null)
			return false;
		if(!valid)error(selects,msnError);
		if(valid)success(selects);
		return valid;
	}
	...
	public static final Boolean numeric(TextField field, Boolean notEmpty, BigDecimal min, BigDecimal max,String msnError) {
		Boolean valid = true;
		try {
	...
	/**
	 * Se encarga de poner color el campo de color rojo alerta de problema y un tooltip sobre el campo indicando el problema.
